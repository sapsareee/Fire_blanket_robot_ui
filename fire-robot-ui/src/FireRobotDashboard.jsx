...
export default function FireRobotDashboard() {
  ...
  const levelStyle = (level) => {
    if (level === "ALERT") return "text-rose-300 border-rose-400/30 bg-rose-500/10";
    if (level === "WARN") return "text-amber-300 border-amber-400/30 bg-amber-500/10";
            ...
            <div className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.15fr)]">
              <section className="rounded-[26px] border border-white/10 bg-slate-900/70 p-4 md:p-5">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    <h3 className="text-base md:text-lg font-semibold">배터리 그래프</h3>
                    <p className="text-sm text-slate-400">실시간 전압 및 잔량 추이</p>
                  </div>
                  <div className="text-right">
                    <div className="text-sm text-slate-400">현재 잔량</div>
                    <div className="text-2xl font-semibold text-emerald-300">45%</div>
                  </div>
                </div>
                <svg viewBox="0 0 320 160" className="h-52 w-full rounded-2xl border border-white/10 bg-slate-950/60 p-2">
                  <defs>
              ...
              <section className="rounded-[26px] border border-white/10 bg-slate-900/70 p-4 md:p-5">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    <h3 className="text-base md:text-lg font-semibold">온도 그래프</h3>
                    <p className="text-sm text-slate-400">내부 온도 및 상승 추세</p>
                  </div>
                  <div className="text-right">
                    <div className="text-sm text-slate-400">최고 온도</div>
                    <div className="text-2xl font-semibold text-amber-300">52°C</div>
                  </div>
                </div>
                <svg viewBox="0 0 320 160" className="h-52 w-full rounded-2xl border border-white/10 bg-slate-950/60 p-2">
                  <defs>
              ...
              <section className="rounded-[26px] border border-white/10 bg-slate-900/70 p-4 md:p-5">
                ...
                <div className="h-52 overflow-auto rounded-2xl border border-white/10 bg-slate-950/60 p-3 space-y-3">
                  {logs.map((log, idx) => (
                    <div key={idx} className="rounded-2xl border border-white/10 bg-white/[0.03] p-3">
                      <div className="mb-2 flex items-center justify-between gap-3">
                        <span className={`rounded-full border px-2 py-1 text-[11px] font-medium ${levelStyle(log.level)}`}>
                          {log.level}
                        </span>
